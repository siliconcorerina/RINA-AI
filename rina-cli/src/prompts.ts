/**
 * Prompt builders — kept in sync with `lsp-server/src/prompts.ts`.
 *
 * The CLI and LSP server share the same prompts so a `rina explain`
 * from the shell produces the same answer as `RINA AI: Explain` from
 * the editor's code-action menu. If you change one, change both.
 *
